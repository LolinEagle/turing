...
let validate_blank blank_str alphabet =
	...
	if not (List.mem blank alphabet) then
		failwith "Blank character must be part of alphabet";
	blank

let validate_finals finals states =
	List.iter (fun final ->
		if not (List.mem final states) then
			failwith (sprintf "Final state '%s' must be part of states" final)
	) finals;
	finals

let validate_initial initial states =
	if not (List.mem initial states) then
		failwith (sprintf "initial state '%s' must be part of states" initial)
	else
		initial
...
let validate_transitions transitions_json states =
	List.iter (fun (state, _) ->
		if not (List.mem state states) then
			failwith (sprintf "Transition '%s' must be part of states" state)
	) transitions_json;
	transitions_json
...
let validate_read read alphabet =
	...
	if not (List.mem read_char alphabet) then
		failwith (sprintf "read value '%c' must be part of alphabet" read_char)
	else
		read_char
...
let validate_write write alphabet =
	...
	if not (List.mem write_char alphabet) then
		failwith (sprintf "write value '%c' must be part of alphabet" write_char)
	else
		write_char

let validate_state state states =
	if not (List.mem state states) then
		failwith (sprintf "state '%s' must be part of states" state)
	else
		state
...
let validate_input alphabet blank input =
	String.iter (fun c ->
		if not (List.mem c alphabet) then
			failwith (sprintf "Input character '%c' must be part of alphabet" c)
